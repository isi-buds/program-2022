...
ca_five_county_covid_data %>% 
  ggplot(aes(x = date, y = scaled_ave_per_pos_tests, color = county)) +
  geom_line(size = 1.3) + 
  scale_color_manual(name = "County", values = county_colors) + 
  theme_minimal(base_size = 15) + 
  labs(
...
joined_case_hospital <- hospital_og %>% 
  ...
  inner_join(ca_five_county_covid_data)
```
...
cummulative_cases_deaths <- ca_covid_data %>%
  group_by(date) %>% 
  summarize(
...
cumulative_cases_plot <- cummulative_cases_deaths %>% 
  ...
  ggtitle("CA Cumulative COVID-19 Cases")

cumulative_deaths_plot <- cummulative_cases_deaths %>% 
  ...
  ggtitle("CA Cumulative COVID-19 Deaths")

gridExtra::grid.arrange(
  cumulative_cases_plot, cumulative_deaths_plot,
  nrow = 2
)
```
